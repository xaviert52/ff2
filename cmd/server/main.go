...
func SeedDatabase(database *gorm.DB) {
  ...
  "steps": {
    ...
    "solicitar_datos": { "id": "solicitar_datos", "type": "FORM", "description": "Ingreso de Cédula, Dactilar, Teléfono, Correo", "schema": { "type": "object", "properties": { "cedula": { "type": "string" }, "codigo_dactilar": { "type": "string" }, "telefono": { "type": "string" }, "correo": { "type": "string" } } }, "transitions": [ { "condition": "true", "next_step": "consultar_rc" } ] },
    "consultar_rc": { "id": "consultar_rc", "type": "ACTION", "connector_id": 99, "config": { "route": "/consulta", "method": "GET", "headers": {"x-user-id": "test_xavier"} }, "input_mapping": { "nui": "{{steps.solicitar_datos.cedula}}", "dactilar": "{{steps.solicitar_datos.codigo_dactilar}}" }, "transitions": [ { "condition": "{{output._status_code}} < 400", "next_step": "descargar_foto_rc" }, { "condition": "{{output._status_code}} >= 400", "next_step": "solicitar_biometria" } ] },
    "descargar_foto_rc": { "id": "descargar_foto_rc", "type": "ACTION", "connector_id": 102, "config": { "route": "/documentos/base64/registro_civil/{{input.cedula}}_foto", "method": "GET" }, "input_mapping": { "cedula": "{{steps.solicitar_datos.cedula}}" }, "transitions": [ { "condition": "true", "next_step": "solicitar_biometria" } ] },
  ...
  "steps": {
    ...
    "descargar_firma_rc": { "id": "descargar_firma_rc", "type": "ACTION", "connector_id": 102, "config": { "route": "/documentos/base64/registro_civil/{{input.cedula}}_firma", "method": "GET" }, "input_mapping": { "cedula": "{{steps.solicitar_cedula_firmas.cedula}}" }, "transitions": [ { "condition": "true", "next_step": "solicitar_captura_firma" } ] },
    ...
    "validar_firma_ia": { "id": "validar_firma_ia", "type": "ACTION", "connector_id": 103, "config": { "route": "/signature-analysis", "method": "POST" }, "input_mapping": { "captured": "data:image/jpeg;base64,{{input.firma_b64}}", "reference": "data:image/jpeg;base64,{{steps.descargar_firma_rc.archivo}}" }, "transitions": [ { "condition": "true", "next_step": "finalizado_firmas" } ] },
